...
class connect_loss(nn.Module):
    ...
    def forward(self, c_map, target):
        # print(c_map.shape,target.shape)
    ...
    def multi_class_forward(self, c_map, target):
        # c_map = c_map * limbus_mask
        # target = target * limbus_mask

        batch_num = c_map.shape[0]
        onehotmask = F.one_hot(target.long(),4)
        onehotmask = onehotmask.permute(0,3,1,2)
        # print(onehotmask.shape)
        ...
        # print(final_pred.shape,vote_out.shape,bicon_map.shape,edge.shape)
    ...
    def single_class_forward(self, c_map, target):
        ...
        batch_num = c_map.shape[0]
        target = target.float()
        con_target = connectivity_matrix(target,self.args.num_class)
        ...
        # print(class_pred.shape,hori_translation.shape,verti_translation.shape)
        pred,bicon_map = Bilateral_voting(class_pred,hori_translation,verti_translation)
        # bicon_map = bicon_map.view(con_target.shape)
        # print(pred.shape,bicon_map.shape,edge.shape)
        # pred = pred.squeeze(1)
        ...
        for j in range(self.args.num_class):
            dice_l += self.dice_loss(pred[:,j,:,:], target[:,j])/self.args.num_class

        # dice_l = self.dice_loss(pred, target)
        
        # print(c_map.shape,con_target.shape)
        # if len(c_map.shape) != 4:
        #     c_map = c_map.unsqueeze(0)
        if len(con_target.shape) != 4:
            con_target = con_target.unsqueeze(0)
        bce_loss = self.BCEloss(pred, target).mean()
        conn_l = self.BCEloss(c_map,con_target).mean()
        bicon_l = self.BCEloss(bicon_map.view(con_target.shape),con_target).mean()

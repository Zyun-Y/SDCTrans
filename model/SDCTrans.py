...
from model.pvtv2_encoder import pvt_v2_b3


class SDCTrans(nn.Module):
    def __init__(self,num_class=1):
        super(SDCTrans,self).__init__()
        ...
        self.fb5=FeatureBlock(512,320,relu=False,last=True) #256
        self.fb4=FeatureBlock(320,128,relu=False) #128
        ...
        self.sb2 = SpaceBlock(512,320,320)
        self.sb3 = SpaceBlock(320,128,128)
        ...
        self.final_decoder=LWdecoder(in_channels=[64,64,128,320],out_channels=32,in_feat_output_strides=(4, 8, 16, 32),out_feat_output_stride=4,norm_fn=nn.BatchNorm2d,num_groups_gn=None)
        ...
        self.final_conv_set = nn.ModuleList()
        for i in range(num_class):
            self.final_conv_set.append(nn.Sequential(
                # nn.ConvTranspose2d(filters[0], 64, 4, 2, 1),
                # nn.ReLU(),
                # nn.Conv2d(64, 48, 3, padding=1),
                # nn.ReLU(),
                LWdecoder(in_channels=[64,64,128,320],out_channels=32,in_feat_output_strides=(4, 8, 16, 32),out_feat_output_stride=4,norm_fn=nn.BatchNorm2d,num_groups_gn=None),

                nn.Conv2d(32, 8, 3, padding=1),
                nn.UpsamplingBilinear2d(scale_factor=2),
                # nn.Conv2d(8, 8, 1)
            ))
        self.space_converter5 = nn.Linear(512, 512)
        self.space_converter4 = nn.Linear(320, 512)
        self.space_converter3 = nn.Linear(128, 320)
        self.space_converter2 = nn.Linear(64, 128)
    ...
    def forward(self, x):
        ...
        d4=self.relu(self.fb5(r5)+c4)  #320
        r4 = self.sb2(self.gap(r5),d4) 

        s5 = self.gap(r5)
        # print(c6.shape)

        #c6: teacher s5:student

        student_5 = self.space_converter5(s5.view(s5.shape[0], s5.shape[1]))
        teacher_5 = c6.view(c6.shape[0], c6.shape[1])
        ...
        #s5: teacher s4:student

        student_4 = self.space_converter4(s4.view(s4.shape[0], s4.shape[1]))
        teacher_4 = s5.view(s5.shape[0], s5.shape[1])


        d2=self.relu(self.fb3(r3)+c2) #64
        r2 = self.sb4(self.gap(r3),d2) 

        #s4: teacher s3:student
        s3 = self.gap(r3)

        student_3 = self.space_converter3(s3.view(s3.shape[0], s3.shape[1]))
        teacher_3 = s4.view(s4.shape[0], s4.shape[1])


        d1=self.fb2(r2)#+c1 #32

        #s3: teacher s2:student
        s2 = self.gap(r2)

        student_2 = self.space_converter2(s2.view(s2.shape[0], s2.shape[1]))
        teacher_2 = s3.view(s3.shape[0], s3.shape[1])

        feat_list = [d1,d2,d3,d4,c5]

        teachers = [teacher_5, teacher_4, teacher_3, teacher_2]
        students = [student_5, student_4, student_3, student_2]

        for i in range(self.num_class):
            # out = self.final_conv_set[i](out)
            out_list.append(self.final_conv_set[i](feat_list))
        # out = self.finalconv2(out)
        # out = self.finalrelu2(out)
        # out = self.finalconv3(out)
        # print(out.shape)
        cls_pred = torch.cat(out_list,dim=1)
        # cls_pred = self.cls_pred_conv_2(final_feat)
        # cls_pred = self.upsample4x_op(cls_pred)

        return cls_pred,mapped_c5, students, teachers
